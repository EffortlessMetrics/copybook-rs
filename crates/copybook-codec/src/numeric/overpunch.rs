...
///
/// ```
/// use copybook_codec::numeric::overpunch::ZeroSignPolicy;
///
...
mod tests {
    ...
    #[test]
    fn test_all_ebcdic_codepage_variants_encode_decode() -> std::result::Result<(), String> {
                ...
                for is_negative in [false, true] {
                    let enc =
                        encode_overpunch_byte(digit, is_negative, cp, ZeroSignPolicy::Positive)
                            .map_err(|error| {
                                format!("{cp:?} digit={digit} neg={is_negative}: {error}")
                            })?;
                    let (d, neg) = decode_overpunch_byte(enc, cp)
                        .map_err(|error| format!("{cp:?} byte=0x{enc:02X}: {error}"))?;
