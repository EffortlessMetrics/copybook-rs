                        ...
                        Ok(0) => {
                            ...
                            if bytes_read == 0 {
                                ...
                                return Err(Error::new(
                                    ...
                                    format!(
                                        "Incomplete fixed record: expected {} bytes, got {} bytes",
                                        lrecl, bytes_read
                                    ),
                                ));
                            }
                ...
                if reserved_bytes != 0 {
                    if self.options.strict_mode {
                        return Err(Error::new(
                            ...
                            format!(
                                "RDW reserved bytes must be zero, found: 0x{:04X}",
                                reserved_bytes
                            ),
                        ));
                    } else {
    ...
    fn decode_next_record(&mut self) -> Result<Option<(Value, u64)>> {
        match self.read_raw_record_with_context()? {
            Some((record_bytes, rdw_header)) => {
                let (json_value, warnings) = crate::decode_record_with_rdw(
                    &self.schema,
                    &record_bytes,
                    rdw_header.as_ref(),
                    &self.options,
    ...
    fn read_raw_record_with_context(&mut self) -> Result<Option<RecordWithContext>> {
        ...
        let (record_data, rdw_header) = match self.options.format {
            ...
            RecordFormat::RDW => {
                // Read RDW header
                let mut rdw_header = [0u8; 4];
                // For RDW, we need to distinguish between clean EOF (no data at all) and truncated header
                let mut bytes_read = 0;
                while bytes_read < 4 {
                    match self.reader.read(&mut rdw_header[bytes_read..]) {
                        Ok(0) => {
                            // EOF reached
                            if bytes_read == 0 {
                                // Clean EOF - no data read at all
                                ...
                                // Incomplete RDW header - some data read but not enough
                                return Err(Error::new(
                                    ErrorCode::CBKR221_RDW_UNDERFLOW,
                                    format!(
                                        "Incomplete RDW header: expected 4 bytes, got {} bytes",
                                        bytes_read
                                    ),
                                ));
                            }
                ...
                if is_ascii_pattern || total_length > 32760 {
                    ...
                    return Err(Error::new(
                        ...
                        format!(
                            "Suspected ASCII corruption in RDW header: length={}, reserved=0x{:04X}",
