...
| Zoned Decimal (PIC 9) | ✅ Fully Supported | `zoned_encoding_format_tests.rs::test_ebcdic_zoned_encoding_detection`, `comprehensive_numeric_tests.rs::test_zoned_decimal_ebcdic_sign_zones_comprehensive`, `cobol_fixture_zoned_encoding_tests.rs::test_simple_fixture_zoned_encoding_integration`, `numeric_evidence_matrix.rs::{string_families_decode_layout_and_roundtrip,zoned_signed_overpunch_decode}` | EBCDIC zones and ASCII overpunch with proper sign handling |
| COMP-3 (Packed Decimal) | ✅ Fully Supported | `comp3_format_verification.rs::test_comp3_format_understanding`, `comprehensive_numeric_tests.rs::test_packed_decimal_comprehensive`, `decimal_edge_cases.rs::test_packed_zero_handling` (512+ property cases in `comp3_property_tests.rs`), `numeric_evidence_matrix.rs::string_families_decode_layout_and_roundtrip` | Nibble sign processing, edge cases, overflow/underflow |
| BINARY (COMP) | ✅ Fully Supported | `comprehensive_numeric_tests.rs::test_binary_signed_unsigned_edges`, `binary_roundtrip_fidelity_tests.rs::test_comp3_packed_decimal_roundtrip_accuracy`, `numeric_comprehensive.rs::test_binary_width_by_digits`, `numeric_evidence_matrix.rs::string_families_decode_layout_and_roundtrip`, `numeric_cli_matrix.rs::cli_decode_comp_binary` (CLI) | Various widths: 1/2/4/8 bytes, signed/unsigned |
| COMP-1/COMP-2 (`comp-1-comp-2`) | ✅ **Fully Supported** | `copybook-core/tests/comp_float_parse_tests.rs`, `copybook-codec/tests/comp_float_codec_tests.rs`, `copybook-codec/tests/comp_float_golden_tests.rs`, `copybook-codec/tests/float_comprehensive.rs` (64 tests), `numeric_evidence_matrix.rs::float_families_decode_layout_and_roundtrip`, `numeric_cli_matrix.rs` (CLI decode/encode + `--float-format`) | IEEE 754 and IBM HFP formats; enabled by default (promoted from experimental) |
...
| VB (Variable-Blocked, beta) | ⚠️ Beta | `e2e_cli_vb_underflow.rs::varied_blocks_short_middle_record_is_per_record_error`, `e2e_cli_vb_underflow.rs::truncated_bdw_is_classified_framing_error`, `e2e_cli_vb_underflow.rs::escaping_rdw_never_reads_past_block_end` | BDW blocks of RDW records; framing holds across blocks, short payloads are per-record `CBKF221`, hostile framing stays classified; lenient short-record acceptance (JRecord) explicitly not adopted |
...
| Feature | Status | Test Evidence | Notes |
|---------|--------|---------------|-------|
| CP037 (US/Canada) | ✅ Fully Supported | Primary codepage across 35+ test files, `comprehensive_numeric_tests.rs::test_codepage_specific_behavior`, `codepage_evidence_matrix.rs::{fixed_and_rdw_decode_agree,decode_rejects_unmappable_byte_all_codepages,encode_capacity_is_measured_after_codepage_encoding,worker_thread_count_does_not_change_ebcdic_decode}` | Default EBCDIC codepage |
| CP273 (German) | ✅ Fully Supported | `prop_codepage_parity_extra.rs`, `decimal_edge_cases.rs::test_zoned_overpunch_by_codepage`, `codepage_evidence_matrix.rs::{fixed_and_rdw_decode_agree,decode_rejects_unmappable_byte_all_codepages,encode_capacity_is_measured_after_codepage_encoding,worker_thread_count_does_not_change_ebcdic_decode}`, `codepage_precedence.rs::cli_decode_cp273_signature` | Full character conversion support |
| CP500 (International) | ✅ Fully Supported | `prop_codepage_parity_extra.rs`, `decimal_edge_cases.rs::test_zoned_overpunch_by_codepage`, `codepage_evidence_matrix.rs::{fixed_and_rdw_decode_agree,decode_rejects_unmappable_byte_all_codepages,encode_capacity_is_measured_after_codepage_encoding,worker_thread_count_does_not_change_ebcdic_decode}`, `codepage_precedence.rs::cli_decode_cp500_signature` | International variant support |
| CP1047 (Latinized) | ✅ Fully Supported | `prop_codepage_parity_extra.rs`, `decimal_edge_cases.rs::test_zoned_overpunch_by_codepage`, `codepage_evidence_matrix.rs::{fixed_and_rdw_decode_agree,decode_rejects_unmappable_byte_all_codepages,encode_capacity_is_measured_after_codepage_encoding,worker_thread_count_does_not_change_ebcdic_decode}`, `codepage_precedence.rs::cli_decode_cp1047_signature` | Latin-1 based EBCDIC |
| CP1140 (Euro) | ✅ Fully Supported | `prop_codepage_parity_extra.rs`, `decimal_edge_cases.rs::test_zoned_overpunch_by_codepage`, `codepage_evidence_matrix.rs::{encode_euro_is_specific_to_cp1140,fixed_and_rdw_decode_agree,decode_rejects_unmappable_byte_all_codepages,worker_thread_count_does_not_change_ebcdic_decode}`, `codepage_precedence.rs::cli_decode_cp1140_signature` | Euro currency support |
