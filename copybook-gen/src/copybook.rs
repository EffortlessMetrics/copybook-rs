    ...
    for i in 1..=field_count {
        ...
        match field_type {
            ...
            1 => {
                ...
                copybook.push_str(&format!("           05  ZONED-{i:02}     PIC {pic}.\n"));
            }
            2 => {
                // Binary field
                let digits = if config.include_edge_cases && rng.gen_bool(0.2) {
                ...
                copybook.push_str(&format!("           05  BINARY-{i:02}    PIC {pic}.\n"));
            }
            3 => {
                // Packed decimal
                let digits = if config.include_edge_cases && rng.gen_bool(0.2) {
                ...
                copybook.push_str(&format!("           05  PACKED-{i:02}    PIC {pic}.\n"));
            }
            4 => {
                // Group with sub-fields
                copybook.push_str(&format!("           05  GROUP-{i:02}.\n"));
...
fn generate_display_heavy_copybook(_rng: &mut StdRng, _config: &GeneratorConfig) -> String {
    ...
    for i in 1..=50 {
        ...
        match field_type {
            0 => {
                copybook.push_str(&format!("           05  TEXT-{i:02}       PIC X(20).\n"));
            }
            1 => {
...
fn generate_comp3_heavy_copybook(_rng: &mut StdRng, _config: &GeneratorConfig) -> String {
    ...
    for i in 1..=10 {
        copybook.push_str(&format!("           05  TEXT-{i:02}       PIC X(15).\n"));
    }
