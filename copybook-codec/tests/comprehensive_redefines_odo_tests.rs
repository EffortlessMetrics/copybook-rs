...
use copybook_core::{parse_copybook, ErrorCode, Occurs};
use copybook_codec::{DecodeOptions, EncodeOptions, RecordFormat, Codepage, JsonNumberMode, RawMode, UnmappablePolicy};
...
"#;
    ...
    assert_eq!(root.children[3].redefines_of, Some("ORIGINAL-FIELD".to_string()));
...
"#;
    ...
    assert!(matches!(array.occurs, Some(Occurs::ODO { min: 1, max: 10, .. })));
...
"#;
    ...
    let summary = copybook_codec::decode_file_to_jsonl(&schema, input, &mut output, &lenient_options).unwrap();
    ...
    let array = json_record.get("VARIABLE-ARRAY").unwrap().as_array().unwrap();
    ...
    assert!(result.is_err(), "Should fail in strict mode with out-of-bounds counter");
}
...
"#;
    ...
    assert!(matches!(outer_array.occurs, Some(Occurs::Fixed { count: 3 })));
    assert_eq!(outer_array.children.len(), 2);
    
    let inner_array = &outer_array.children[0];
    assert!(matches!(inner_array.occurs, Some(Occurs::Fixed { count: 2 })));
...
"#;
    ...
    let next_field = root.children.iter().find(|f| f.name == "NEXT-FIELD").unwrap();
...
"#;
    ...
    let summary = copybook_codec::decode_file_to_jsonl(&schema, input, &mut output, &options).unwrap();
...
"#;
    ...
    let keys: Vec<&str> = json_record.as_object().unwrap().keys().map(|s| s.as_str()).collect();
    let expected_order = vec!["ORIGINAL", "THIRD-REDEFINE", "FIRST-REDEFINE", "SECOND-REDEFINE"];
    
    for (i, expected_key) in expected_order.iter().enumerate() {
        assert!(keys.iter().position(|&k| k == *expected_key).unwrap() == i,
                "Field {} not in expected position", expected_key);
    }
}