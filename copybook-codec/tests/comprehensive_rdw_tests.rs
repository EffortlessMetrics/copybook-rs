...
fn test_rdw_suspect_ascii_heuristic() {
    ...
    if let Ok(summary) = result {
        assert!(summary.has_warnings(), "Should have CBKF104_RDW_SUSPECT_ASCII warning");
    } else {
...
fn test_rdw_zero_length_record() {
    ...
    if schema.lrecl_fixed.unwrap_or(0) == 0 {
        assert!(result.is_ok(), "Zero-length record should be valid for empty schema");
    } else {
...
fn test_rdw_big_endian_length() {
    ...
    assert_eq!(record_value.len(), 256);
...
fn test_rdw_maximum_length_handling() {
    ...
    if result.is_ok() {
        ...
        println!("Maximum length test failed (acceptable): {:?}", result.unwrap_err());
    }
}
