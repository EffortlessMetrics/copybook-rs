            ...
            benchmarks: vec![
                MockBenchmarkResult {
                    ...
                    throughput: Some(MockThroughput {
                        ...
                        value: 4.22 * 1024.0 * 1024.0 * 1024.0, // 4.22 GiB/s in bytes/sec
                    }),
                    mean_time: Duration::from_nanos(950_000),
                    confidence_interval: (
                        Duration::from_nanos(940_000),
                        Duration::from_nanos(960_000),
                    ),
                    sample_count: 100,
                },
                MockBenchmarkResult {
...
impl PerformanceExtractor {
    pub fn extract_display_throughput_gibs(
        results: &MockCriterionResults,
    ) -> Result<f64, Box<dyn std::error::Error>> {
        let display_benchmarks: Vec<_> = results
            .benchmarks
            .iter()
        ...
        Ok(total_throughput / f64::from(count))
    }

    pub fn extract_comp3_throughput_mibs(
        results: &MockCriterionResults,
    ) -> Result<f64, Box<dyn std::error::Error>> {
        let comp3_benchmarks: Vec<_> = results
            .benchmarks
            .iter()
...
fn test_benchmark_execution_environment_readiness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        perf_enabled,
        "PERF environment variable test value must be parseable"
    );
    ...
    assert!(
        manifest_dir.exists(),
        "copybook-bench manifest directory must exist"
    );
    assert!(
        manifest_dir.is_dir(),
        "Manifest directory must be a directory"
    );

    let cargo_toml = manifest_dir.join("Cargo.toml");
    assert!(
        cargo_toml.exists(),
        "Cargo.toml must exist for benchmark execution"
    );
...
fn test_display_throughput_extraction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        display_gibs > 4.0,
        "DISPLAY throughput must exceed enterprise minimum"
    );
    assert!(
        display_gibs < 10.0,
...
fn test_comp3_throughput_extraction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        comp3_mibs > 500.0,
        "COMP-3 throughput must exceed enterprise minimum"
    );
    assert!(
        comp3_mibs < 1000.0,
...
fn test_json_output_generation_from_benchmarks() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ...
        "JSON must contain display_gibs field"
    );
    assert!(
        obj.contains_key("comp3_mibs"),
        "JSON must contain comp3_mibs field"
    );
    assert!(
...
fn test_benchmark_infrastructure_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if bench_dir.exists() {
        ...
        assert!(
            decode_performance.exists() || comp3_bench.exists(),
    ...
    assert!(
        cargo_toml_content.contains("criterion"),
...
fn test_performance_threshold_enforcement() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        warnings.is_empty(),
        "Current performance should not generate warnings"
    );
    assert!(
        errors.is_empty(),
        "Current performance should not generate errors"
    );
    ...
    assert!(
        poor_display < display_floor_gibs,
        "Test case should be below DISPLAY floor"
    );
    assert!(
        poor_comp3 < comp3_floor_mibs,
        "Test case should be below COMP-3 floor"
    );
...
fn test_statistical_properties_extraction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let avg_confidence_width: f64 = mock_results
        .benchmarks
        .iter()
        .map(|b| {
        ...
        .sum::<f64>()
        / mock_results.benchmarks.len() as f64;
    ...
    assert!(
        avg_confidence_width < 0.05,
        "Confidence intervals must be tight for precision"
    );

    // Verify benchmark execution time is reasonable
    assert!(
        mock_results.execution_time.as_secs() > 10,
        "Benchmarks must run long enough for accuracy"
    );
    assert!(
        mock_results.execution_time.as_secs() < 600,
        "Benchmarks must complete in reasonable time"
    );
...
#[test]
fn test_enterprise_cobol_processing_performance_measurement()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        has_display,
        "Must include DISPLAY processing benchmarks for COBOL compatibility"
    );
    ...
    assert!(
        has_comp3,
        "Must include COMP-3 processing benchmarks for COBOL compatibility"
    );

    // Validate enterprise-scale data sizes (4GB, 2GB scenarios)
    let has_large_data = benchmark_names
        .iter()
        .any(|name| name.contains("4gb") || name.contains("2gb"));
    assert!(
        has_large_data,
        "Must include enterprise-scale data size benchmarks"
    );

    // Verify throughput measurements are available
    for benchmark in &mock_results.benchmarks {
        assert!(
            benchmark.throughput.is_some(),
            "Benchmark {} must have throughput measurement",
            benchmark.name
        );
