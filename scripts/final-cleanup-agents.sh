...
run_cargo() {
  ...
  elif command -v rustup.exe >/dev/null 2>&1 && rustup.exe run stable cargo --version >/dev/null 2>&1; then
...
run_cargo run --quiet --manifest-path "$ROOT_DIR/tools/copybook-scripts/Cargo.toml" -- final-cleanup-agents "$@"
