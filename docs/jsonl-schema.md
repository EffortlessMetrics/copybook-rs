...
When a flattened child collides with an existing field in the enclosing map, the flattened key
uses the deterministic `__dupN` suffix convention rather than overwriting the existing value.
