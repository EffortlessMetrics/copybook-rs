...
pub fn run(check: bool) -> Result<()> {
    ...
    .with_context(|| "copying ripr+ badge endpoint into badges/".to_string())?;
...
pub fn test_efficiency_report() -> Result<()> {
    ensure_test_efficiency_report(&workspace_root_path())?;
...
fn compare_files(committed: &Path, generated: &Path) -> Result<()> {
    ...
    if committed_bytes != generated_bytes {
        bail!(
            "badge endpoint drift detected: {} differs from {}\nrun `cargo xtask badges` to refresh committed endpoint JSON",
            committed.display(),
            generated.display()
        );
    }
