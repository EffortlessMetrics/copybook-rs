...
"#;
    ...
    if let Some(context) = &error.context {
        ...
        assert!(context.record_index.is_some() || context.field_path.is_some() || context.byte_offset.is_some());
    }
}
...
"#;
    ...
    assert!(error.message.contains("missing") || error.message.contains("COUNTER") || error.message.contains("required"));
}