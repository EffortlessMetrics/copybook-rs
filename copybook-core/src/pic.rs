...
impl PicClause {
    ...
    ///
    /// Returns an error if:
    /// - The PIC string contains unsupported edited patterns (`CBKP051_UNSUPPORTED_EDITED_PIC`)
    /// - The PIC string format is invalid or cannot be parsed (`CBKP001_SYNTAX`)
    ///
    /// # Panics
    ///
    /// May panic if the repeat count parsing fails in unexpected ways due to malformed input
        ...
        while let Some(ch) = chars.next() {
            match ch.to_ascii_uppercase() {
                ...
                '(' => {
                    ...
                    if found_v {
                        scale = scale.saturating_sub(1) + i16::try_from(count).unwrap_or(i16::MAX);
                    }
                }
                ' ' | '\t' => {
                    // Skip whitespace
        ...
        match self.kind {
            ...
            PicKind::NumericDisplay => {
                if self.scale == 0 {
                    ...
                    let integer_digits = self
                        .digits
                        .saturating_sub(u16::try_from(self.scale).unwrap_or(0));
