...
//! RDW (Record Descriptor Word) header primitives and low-level read helpers.
...
mod tests {
    ...
    #[test]
    fn rdw_header_extracts_length_and_reserved() {
    ...
    #[test]
    fn rdw_header_rejects_payloads_larger_than_u16() {
    ...
    #[test]
    fn rdw_ascii_corruption_heuristic_flags_ascii_digits() {
        assert!(rdw_is_suspect_ascii_corruption([b'1', b'2', 0, 0]));
        assert!(!rdw_is_suspect_ascii_corruption([0, 12, 0, 0]));
    }

    #[test]
    fn rdw_try_peek_len_none_for_less_than_two_bytes() {
    ...
    fn rdw_read_len_consumes_two_bytes() {
        let mut cur = Cursor::new(vec![0x01, 0x02, 0xAA, 0xBB, 0xCC]);
    ...
    fn rdw_slice_body_short_is_cbkf102() {
        let mut cur = Cursor::new(vec![0x00, 0x04, 0x11, 0x22]);
        let len = rdw_read_len(&mut cur).unwrap();
