...
use copybook_core::{Error, ErrorCode, ErrorContext, Result, Field, Occurs, Schema};
...
pub fn validate_odo_counter(
    ...
    if strict_mode {
        ...
        return Err(Error::new(ErrorCode::CBKS301_ODO_CLIPPED, error_msg)
            .with_context(ErrorContext {
                record_index: Some(record_index),
                field_path: Some(field_path.to_string()),
                byte_offset: Some(byte_offset),
                line_number: None,
                details: Some(format!("counter_field={}, counter_value={}", counter_path, counter_value)),
            }));
    }

    // In lenient mode, clamp and warn (NORMATIVE)
    let (actual_count, error_code, action) = if counter_value < min_count {
        (min_count, ErrorCode::CBKS302_ODO_RAISED, "raised to minimum")
    } else {
        (max_count, ErrorCode::CBKS301_ODO_CLIPPED, "clipped to maximum")
    };
    ...
    let warning = Error::new(
        ...
        byte_offset: Some(byte_offset),
        line_number: None,
        details: Some(format!("counter_field={}, original_value={}, clamped_value={}", 
...
pub fn validate_odo_tail_position(
    ...
    debug!("Validating ODO tail position: array={}, counter={}", odo_field_path, counter_field_path);

    // Find the ODO field and its parent group
    let odo_field = schema.find_field(odo_field_path)
    ...
    let _odo_info = match &odo_field.occurs {
        Some(Occurs::ODO { min: _, max: _, counter_path: _ }) => (),
    ...
    let counter_field = schema.find_field(counter_field_path)
        .ok_or_else(|| Error::new(
            ErrorCode::CBKS121_COUNTER_NOT_FOUND,
            format!("ODO counter field '{}' not found in schema", counter_field_path)
        ))?;
    ...
    debug!("ODO tail position validation passed for array '{}' with counter '{}'", 
...
pub fn build_redefines_context(schema: &Schema, json_data: &Value) -> RedefinesContext {
    ...
    debug!("Built REDEFINES context: {} clusters, {} field mappings", 
           context.cluster_views.len(), context.field_to_cluster.len());
...
pub fn validate_redefines_encoding(
    ...
    debug!("Validating REDEFINES encoding: cluster={}, field={}, use_raw={}", 
           cluster_path, field_path, use_raw);

    // Check for raw data precedence (step 1)
    if use_raw {
        if let Value::Object(obj) = json_data {
            if let Some(Value::String(_)) = obj.get("__raw_b64") {
    ...
    let non_null_views = context.cluster_views.get(cluster_path)
        .map(|views| views.len())
        .unwrap_or(0);
    ...
    if non_null_views == 1 {
        debug!("Single non-null view found for REDEFINES cluster '{}'", cluster_path);
        return Ok(());
    }

    // Multiple non-null views - ambiguous write (step 3)
    let views_list = context.cluster_views.get(cluster_path)
    ...
    Err(Error::new(
        ...
        byte_offset: Some(byte_offset),
        line_number: None,
        details: Some(format!("cluster_path={}, non_null_views={}", cluster_path, non_null_views)),
    }))
}
...
pub fn handle_missing_counter_field(
    ...
    debug!("Handling missing counter field: counter={}, array={}", counter_path, array_path);
    ...
    let details = if suggestions.is_empty() {
        format!("array_field={}, searched_paths=all_schema_fields", array_path)
    } else {
        format!("array_field={}, similar_fields=[{}]", array_path, suggestions.join(", "))
    };
...
fn collect_redefines_from_fields(fields: &[Field], context: &mut RedefinesContext) {
    for field in fields {
        if let Some(ref target) = field.redefines_of {
            ...
            context.field_to_cluster.insert(field.name.clone(), target.clone());
            
            // Initialize cluster if not present (use target name, not full path)
            let target_name = target.split('.').last().unwrap_or(target);
            context.cluster_views.entry(target_name.to_string()).or_insert_with(Vec::new);
        }
    ...
    for (key, value) in json_obj {
        ...
        if let Some(cluster_path) = context.field_to_cluster.get(key) {
            if !value.is_null() {
                ...
                let views = context.cluster_views.entry(cluster_path.clone()).or_insert_with(Vec::new);
            ...
            if context.cluster_views.contains_key(key) && !value.is_null() {
                ...
                let views = context.cluster_views.entry(key.clone()).or_insert_with(Vec::new);
                if !views.contains(key) {
                    views.push(key.clone());
                }
            }
...
pub fn validate_odo_encode(
    ...
    if validation_result.was_clamped {
        ...
        return Err(Error::new(
            ...
            field_path,
            byte_offset,
            Some(format!("counter_field={}, array_length={}", counter_path, array_length)),
        )));
    }
...
mod tests {
    ...
    fn test_odo_validation_within_bounds() {
        let result = validate_odo_counter(
            3, 0, 5, "ROOT.ARRAY", "ROOT.COUNTER", 1, 3, false
        ).unwrap();
    ...
    fn test_odo_validation_strict_mode_over_max() {
        let result = validate_odo_counter(
            10, 0, 5, "ROOT.ARRAY", "ROOT.COUNTER", 1, 3, true
        );
    ...
    fn test_odo_validation_lenient_mode_clamp_max() {
        let result = validate_odo_counter(
            10, 0, 5, "ROOT.ARRAY", "ROOT.COUNTER", 1, 3, false
        ).unwrap();
    ...
    fn test_odo_validation_lenient_mode_raise_min() {
        let result = validate_odo_counter(
            0, 1, 5, "ROOT.ARRAY", "ROOT.COUNTER", 1, 3, false
        ).unwrap();
    ...
    fn test_odo_tail_position_validation() {
        ...
        let result = validate_odo_tail_position(
            &schema, 
            "ROOT.ARRAY", 
            "ROOT.COUNTER"
        );
