...
fn safe_ops_and_safe_text_contract() {
    assert_eq!(
        parse_usize("2048", "contract").expect("usize parse"),
        safe_ops_parse_usize("2048", "contract").expect("usize parse")
    );
    assert_eq!(
        parse_isize("-42", "contract").expect("isize parse"),
        safe_ops_parse_isize("-42", "contract").expect("isize parse")
    );
    assert_eq!(
        safe_parse_u16("65535", "contract").expect("u16 parse"),
        safe_ops_parse_u16("65535", "contract").expect("u16 parse")
    );

    let text = "xyz";
    assert_eq!(
        safe_string_char_at(text, 1, "contract").expect("char"),
        safe_ops_char_at(text, 1, "contract").expect("char")
    );
