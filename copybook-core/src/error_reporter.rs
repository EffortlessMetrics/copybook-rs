...
impl ErrorReporter {
    ...
    /// # Errors
    ///
    /// Returns an error if the severity indicates processing should stop or maximum errors exceeded
    ///
    /// # Panics
    ///
    /// Panics if `max_errors` is set but `unwrap()` is called on it when it should be `Some`
