...
fn test_zoned_decimal_ebcdic_sign_zones_comprehensive() {
    ...
    for (data, expected, description) in positive_tests {
        ...
        assert_eq!(json_record["SIGNED-FIELD"], expected, "Failed for {}", description);
    }
...
fn test_packed_decimal_sign_nibbles_comprehensive() {
    ...
    for (data, expected, description) in sign_tests {
        ...
        assert_eq!(json_record["SIGNED-PACKED"], expected, "Failed for {}", description);
    }
}
...
"#;
    ...
    // Test maximum values
    // 16-bit: max unsigned = 65535, max signed = 32767
    // 32-bit: max unsigned = 4294967295, max signed = 2147483647
...
fn test_throughput_measurement() {
    ...
    println!("Processed {} records in {:?} at {:.2} MB/s", 
