...
fn test_redefines_shorter_overlay() {
    ...
    let short_redefines = schema.fields.iter()
    ...
    assert_eq!(short_redefines.redefines_of.as_ref().unwrap(), "ORIGINAL-FIELD");
    
    // Should be zoned decimal
    assert!(matches!(short_redefines.kind, FieldKind::ZonedDecimal { digits: 10, scale: 0, signed: false }));
}
...
fn test_redefines_equal_overlay() {
    ...
    let equal_redefines = schema.fields.iter()
    ...
    assert_eq!(equal_redefines.redefines_of.as_ref().unwrap(), "ORIGINAL-FIELD");
}
...
fn test_redefines_longer_overlay() {
    ...
    let long_redefines = schema.fields.iter()
    ...
    let next_field = schema.fields.iter()
...
fn test_redefines_raw_data_precedence() {
    ...
    let decode_summary = copybook_codec::decode_file_to_jsonl(&schema, input, &mut decode_output, &decode_options).unwrap();
    ...
    let result = copybook_codec::encode_jsonl_to_file(&schema, input, &mut encode_output, &encode_options);
    assert!(result.is_ok());
...
fn test_redefines_round_trip_preservation() {
    ...
    copybook_codec::decode_file_to_jsonl(&schema, input, &mut decode_output, &decode_options).unwrap();
    ...
    copybook_codec::encode_jsonl_to_file(&schema, input, &mut encode_output, &encode_options).unwrap();
