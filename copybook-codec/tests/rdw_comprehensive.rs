...
fn test_rdw_raw_preservation_with_reserved() {
    ...
    let decode_result = copybook_codec::decode_file_to_jsonl(&schema, input, &mut decode_output, &decode_options);
    ...
    let encode_result = copybook_codec::encode_jsonl_to_file(&schema, input, &mut encode_output, &encode_options);
