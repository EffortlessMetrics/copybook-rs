...
## Platform Support
- Rust-native maintenance automation lives in `tools/copybook-scripts`.
- Shell scripts (.sh) are retained where they are needed as Unix entrypoints or command orchestrators.
- Batch files (.bat) are retained where Windows-native entrypoints are needed.
