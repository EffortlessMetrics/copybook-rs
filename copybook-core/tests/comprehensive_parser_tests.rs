...
fn test_edited_pic_error_normative() {
    ...
    for (edited_pic, description) in edited_pics {
        ...
        assert!(result.is_err(), "Should fail for {}: {}", description, edited_pic);
...
fn test_sign_clause_as_edited_pic_normative() {
    ...
    let sign_clauses = vec![
        ...
        ("01 FIELD3 PIC S999 SIGN LEADING SEPARATE.", "SIGN LEADING SEPARATE"),
        ("01 FIELD4 PIC S999 SIGN TRAILING SEPARATE.", "SIGN TRAILING SEPARATE"),
    ];
    
    for (sign_clause, description) in sign_clauses {
        let result = parse_copybook(sign_clause);
        assert!(result.is_err(), "Should fail for {}: {}", description, sign_clause);
...
fn test_valid_pic_clauses_comprehensive() {
    ...
    let valid_pics = vec![
        ("01 FIELD1 PIC X(10).", "Alphanum", FieldKind::Alphanum { len: 10 }),
        ("01 FIELD2 PIC 9(5).", "Zoned unsigned", FieldKind::ZonedDecimal { digits: 5, scale: 0, signed: false }),
        ("01 FIELD3 PIC S9(5).", "Zoned signed", FieldKind::ZonedDecimal { digits: 5, scale: 0, signed: true }),
        ("01 FIELD4 PIC 9(3)V99.", "Zoned with scale", FieldKind::ZonedDecimal { digits: 5, scale: 2, signed: false }),
        ("01 FIELD5 PIC S9(3)V99.", "Zoned signed with scale", FieldKind::ZonedDecimal { digits: 5, scale: 2, signed: true }),
        ("01 FIELD6 PIC 9(5) COMP.", "Binary", FieldKind::BinaryInt { bits: 32, signed: false }),
        ("01 FIELD7 PIC S9(5) COMP.", "Binary signed", FieldKind::BinaryInt { bits: 32, signed: true }),
        ("01 FIELD8 PIC 9(5) COMP-3.", "Packed", FieldKind::PackedDecimal { digits: 5, scale: 0, signed: false }),
        ("01 FIELD9 PIC S9(5) COMP-3.", "Packed signed", FieldKind::PackedDecimal { digits: 5, scale: 0, signed: true }),
    ];
    
    for (copybook, description, expected_kind) in valid_pics {
        let result = parse_copybook(copybook);
        assert!(result.is_ok(), "Should succeed for {}: {}", description, copybook);
        ...
        match (&schema.fields[0].kind, &expected_kind) {
            ...
            (FieldKind::ZonedDecimal { digits: d1, scale: s1, signed: sg1 }, 
             FieldKind::ZonedDecimal { digits: d2, scale: s2, signed: sg2 }) => {
                assert_eq!(d1, d2);
                assert_eq!(s1, s2);
                assert_eq!(sg1, sg2);
            },
            (FieldKind::BinaryInt { bits: b1, signed: s1 }, 
             FieldKind::BinaryInt { bits: b2, signed: s2 }) => {
                assert_eq!(b1, b2);
                assert_eq!(s1, s2);
            },
            (FieldKind::PackedDecimal { digits: d1, scale: s1, signed: sg1 }, 
             FieldKind::PackedDecimal { digits: d2, scale: s2, signed: sg2 }) => {
                assert_eq!(d1, d2);
                assert_eq!(s1, s2);
                assert_eq!(sg1, sg2);
            },
            _ => panic!("Field kind mismatch for {}: expected {:?}, got {:?}", 
...
"#;
    ...
    assert!(matches!(simple_array.occurs, Some(Occurs::Fixed { count: 5 })));
    
    // Check nested group array
    let nested_group = &root.children[1];
    assert_eq!(nested_group.name, "NESTED-GROUP");
    assert!(matches!(nested_group.occurs, Some(Occurs::Fixed { count: 3 })));
...
"#;
    ...
    assert!(matches!(odo_array.occurs, Some(Occurs::ODO { min: 1, max: 10, .. })));
...
"#;
    ...
    assert_eq!(root.children[1].redefines_of, Some("ORIGINAL-FIELD".to_string()));
    
    assert_eq!(root.children[2].name, "ANOTHER-REDEFINE");
    assert_eq!(root.children[2].redefines_of, Some("ORIGINAL-FIELD".to_string()));
