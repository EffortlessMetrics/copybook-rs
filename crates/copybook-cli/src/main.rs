...
Field Projection:\n\
  ...
  ODO counters and parent groups are automatically included."
    ...
    Decode {
        ...
        /// paths and on encode paths (`CBKF226_RECORD_BOUND_EXCEEDED`).
        #[arg(long, value_name = "PROFILE")]
        profile: Option<PathBuf>,
...
Field Projection:\n\
  ...
  ODO counters and parent groups are automatically included."
    ...
    Encode {
        ...
        /// unchanged. `limits.maximum_record_length` is enforced on
        /// encode paths (`CBKF226_RECORD_BOUND_EXCEEDED`).
        #[arg(long, value_name = "PROFILE")]
        profile: Option<PathBuf>,
        /// Record format (explicit, no auto-detection). Required unless --profile supplies framing.
        #[arg(long, required_unless_present = "profile")]
...
Field Projection:
  ...
  ODO counters and parent groups are automatically included.")]
    Verify {
        ...
        /// decode/verify read paths and on encode paths
...
fn run() -> anyhow::Result<ExitCode> {
    ...
    // Handle feature flags: the CLI resolves CLI args, env vars, and config
    // files into one explicit value here (#656 Phase D) and passes it down
    // to every command. No process-global flag state exists anymore.
