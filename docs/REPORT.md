...
project status reference. Release-readiness policy posture is the intersection of
`ROADMAP.md`, `docs/PRODUCTION_READINESS.md`, and the release gate
workflow controls in `.github/workflows/release-readiness.yml`/`.github/workflows/perf.yml`.
...
### RC Gate (Outstanding Incomplete Work)

| Item | Status | Owner | Timestamp | Evidence Required |
|---|---|---|---|---|
| `TODO(AC5)` (`copybook-core/src/audit/security.rs`) | pass | @EffortlessSteven | 2026-02-28 | (status=pass; owner=@EffortlessSteven; evidence=[security audit implementation](copybook-core/src/audit/security.rs), [enterprise security path](copybook-cli/src/commands/audit.rs), [AC5 gate contract](copybook-core/src/audit/mod.rs), [security gate tests](copybook-core/src/audit/security.rs)) |
| `TODO(AC10)` (`copybook-core/src/audit/mod.rs`) | pass | @EffortlessSteven | N/A | (status=pass; owner=@EffortlessSteven; evidence=Fail-fast health selectors in `validate_health_check_request`, execution evidence guard in `audit_core::require_health_execution_evidence`, and regression coverage in `test_run_audit_health_check_rejects_empty_inputs_for_requested_checks`.) |
| `TODO(AC11)` (`copybook-core/src/audit/performance.rs`) | pass | @EffortlessSteven | 2026-02-28 | (status=pass; owner=@EffortlessSteven; evidence=[AC11 objective checks](copybook-cli/src/commands/audit.rs), [performance threshold model](copybook-core/src/audit/performance.rs)) |
...
| `TODO(AC17)` (`copybook-core/src/audit/security.rs`) | warn | @EffortlessSteven | N/A | Expand deterministic tests for production-path failures, fallbacks, and recovery scenarios (current coverage includes `test_security_auditor_audit_access_event_requires_enterprise_gate`, `test_run_security_audit_requires_enterprise_gate`, `test_run_security_audit_succeeds_when_enterprise_gate_enabled`, `test_run_security_audit_succeeds_with_real_time_monitoring_when_ac9_gate_enabled`, `test_run_audit_command_dispatch_requires_ac17_enterprise_gate`, and `test_run_audit_command_dispatch_succeeds_when_ac17_gate_enabled`). |
| `TODO(AC9)` (`copybook-cli/src/commands/audit.rs`) | warn | @EffortlessSteven | N/A | AC9 monitoring remains stub/diagnostic only; production streaming path is still pending replacement. AC9 status-contract is tested by `test_run_security_audit_reports_ac9_enabled_status_when_real_time_monitoring_requested` and `test_run_security_audit_reports_ac9_enabled_status_when_threat_assessment_requested`, but event ingestion/fanout replacement is still required. |
...
Release-candidate promotion should not claim enterprise monitoring completeness
until AC9 stub-mode is replaced with full streaming infrastructure and
production-evidence collection is updated.

- **Perf evidence pending**: Release readiness blocks without valid non-WSL replay evidence.
- Required evidence is `artifacts/perf/non-wsl/perf.json` containing status `available` or `available-from-cache`, plus baseline date and checksum.
...
- **Wave-5 evidence**:
  - **DISPLAY**: `1349.6524445004138 MiB/s` (2025-12-29, `display_heavy_slo_80mbps`)
  - **COMP-3**: `16.24215679348751 MiB/s` (2025-12-29, `comp3_heavy_slo_40mbps`)
  - **Non-WSL baseline replay**: pending
  (status=warn; owner=@EffortlessSteven; evidence=run a canonical non-WSL replay into `artifacts/perf/non-wsl/perf.json`, include execution timestamp and checksum, and require a variance envelope comparable to current WSL baseline before RC promotion.)
...
- ⚠️ **AC9 monitoring**: AC9 monitoring is intentionally non-production/stub-only and must not be treated as production evidence. Real-time audit monitoring is still stubbed behind the
  opt-in `deterministic_audit_stubs` feature and is not production-complete.
  (status=warn; owner=@EffortlessSteven; evidence=This remains stub validation only and should not be treated as production monitoring evidence until streaming path replaces stubs.)
- ✅ **COBOL Completeness**: COMP-1/COMP-2 and SIGN SEPARATE are fully supported
  ...
  with current E1-E3 feature coverage; nested ODO is partially supported (O1-O4
  supported; O5/O6 rejected by design; O7 rejected by RENAMES R4-R6 policy);
